...
string versionString = "0.1.0";

#ifdef CONFIG_LUA_CHECKER
...
void writefile(char* content, string filename, string base,
    ...
    return;
#elif defined(__linux__) && defined(CONFIG_LUA_CHECKER)
...
void local_unpack(string _datpath) {
    ...
    path.append(idxpath);
    path.append("/");
    dounpack(fin, idxpath, datpath, path);
}
...
void download_update(const char* argv)
{
    ...
    if (curl) {
        ...
        if (url == "error") {
            ...
            curl_easy_setopt(curl, CURLOPT_URL, ss2.str().c_str());
